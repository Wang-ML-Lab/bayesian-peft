...
for dataset in winogrande_s ARC-Challenge ARC-Easy winogrande_m obqa boolq; do
  for sample in 10; do
    for seed in 1 2 3; do
      device=$((index % 3))
      index=$((index + 1))
      name=$modelwrapper-map-th$th-$dataset-sample$sample-beta$beta-iter$iter-seed$seed
      CUDA_VISIBLE_DEVICES=$device python run/main.py --dataset-type mcdataset --dataset $dataset \
        ...
        --load-lora-path map/meta-llama/Meta-Llama-3.1-8B/$dataset/map-$dataset-sample10-eps0.05-kllr0.0075-beta0.15-seed$seed \
