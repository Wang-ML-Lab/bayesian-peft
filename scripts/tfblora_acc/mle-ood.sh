...
for dataset in ARC-Challenge ARC-Easy MMLU-chem MMLU-phy; do
  for sample in 10; do
    for seed in 1 2 3; do
      ...
      device=2
      name=$modelwrapper-mle-ood-th$th-$dataset-sample$sample-seed$seed
      CUDA_VISIBLE_DEVICES=$device python run/main.py --dataset-type mcdataset --dataset $dataset \
        ...
        --seed $seed \
        --evaluate \
        --wandb-name $name --wandb-project "var-infer-adaprior-ood-$dataset" \
        --apply-classhead-lora --lora-r 8 --lora-alpha 16 --lora-dropout 0 \
        --log-path $name \
        --max-train-steps 0 \
        --eval-per-steps 6000 \
        --bayes-klreweighting \
        --load-lora-path /data/local/yibin/bayesian-peft/mle/meta-llama/Meta-Llama-3.1-8B/$ori_dataset/mle-$ori_dataset-seed$seed \
