...
def blob_linear_forward(self, x: torch.Tensor, *args: Any, **kwargs: Any):
    ...
    for active_adapter in self.active_adapters:
        ...
        if self.blobsample:
            ...
            x = x.to(lora_A.weight.dtype)
            if x.dim() == 2:
                r_A = torch.ones((x.size(0), self.in_features), device=x.device, dtype=x.dtype).uniform_(-1, 1).sign()
                s_A = torch.ones((x.size(0), self.r[active_adapter]), device=x.device, dtype=x.dtype).uniform_(-1, 1).sign()
            else:
                r_A = torch.ones((x.size(0), x.size(1), self.in_features), device=x.device, dtype=x.dtype).uniform_(-1, 1).sign()
                s_A = torch.ones((x.size(0), x.size(1), self.r[active_adapter]), device=x.device, dtype=x.dtype).uniform_(-1, 1).sign()


            x = dropout(x)
            lora_noise_a = A_sigma * torch.randn_like(self.lora_A[active_adapter].weight)

            noise = (((x * r_A) @ lora_noise_a.transpose(0, 1)) * s_A) @ self.lora_B[active_adapter].weight.transpose(0, 1)
...
def div_posterior_prior(self) -> torch.Tensor:
    def kl_div_stable(mu_q, sigma_q, mu_p, sigma_p):
        eps = 1e-6
        kl = math.log(sigma_p+eps) - torch.log(sigma_q.to(torch.float64)+eps) + (sigma_q.to(torch.float64)**2 + (mu_q.to(torch.float64) - mu_p)**2) / (2 * (sigma_p**2)+eps) - 0.5
        return kl.sum()
    kl = 0
    for active_adapter in self.active_adapters:
        if self.bayes_eps < 0:
...
class BLoB(WrapperBase):
    ...
    def _modify_lora_layers(self, module):
        ...
        for name, child in module.named_children():
            if isinstance(child, LoraLayer) and isinstance(child, Linear):
                ...
                setattr(child, 'forward', blob_linear_forward.__get__(child, child.__class__))
                # add new methods
            ...
            if isinstance(child, LoraLayer) and isinstance(child, Linear8bitLt):
                self._wrap_lora_layer(child)
                # modify existing methods
                setattr(child, 'forward', blob_8bitlinear_forward.__get__(child, child.__class__))
                # add new methods
                setattr(child, 'div_posterior_prior', div_posterior_prior.__get__(child, child.__class__))
                setattr(child, 'sample', sample.__get__(child, child.__class__))
            else:
    ...
    def _wrap_lora_layer(self, lora_layer):
        ...
        for adapter_name in lora_layer._active_adapter:
            lora_layer.lora_A_rho[adapter_name] = nn.Parameter(lora_layer.lora_A[adapter_name].weight.new_zeros(lora_layer.r[adapter_name], lora_layer.in_features))

        if adapter_name in lora_layer.lora_A.keys():
            if lora_layer.bayes_eps < 0:
                nn.init.uniform_(lora_layer.lora_A_rho[adapter_name], lora_layer.bayes_eps-1, lora_layer.bayes_eps)
            else:
                nn.init.uniform_(lora_layer.lora_A_rho[adapter_name], lora_layer.bayes_eps/math.sqrt(2), lora_layer.bayes_eps)
    ...
    def evaluate(self, eval_loader):
        ...
        for step, batch in enumerate(eval_loader):
            with torch.no_grad() and torch.inference_mode():
                logits = self.forward_logits(batch, sample = not self.args.bayes_inference_notsample, n_samples=self.eval_n_samples).detach()
                if self.args.dataset_type == 'mcdataset':
                    _, labels, _ = batch
                else:
                    labels = batch["labels"]
    ...
    def prepare_for_fit_evaluate(self, dataset, wandb_logger=None):
        ...
        if self.args.bayes_klreweighting:
